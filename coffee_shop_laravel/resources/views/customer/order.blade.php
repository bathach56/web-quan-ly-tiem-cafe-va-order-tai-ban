...
<head>
    ...
    <title>{{ __('messages.menu_title') }} | {{ $shop_setting->shop_name ?? 'HUTECH Coffee' }}</title>
    ...
    <style>
        ...
        h1, h2, h3, h4, h5, .modal-title, .header h6 {
...
<div class="header">
    <div class="d-flex justify-content-between align-items-center">
        ...
        <div class="dropdown">
            <div class="rounded-circle border d-flex align-items-center justify-content-center bg-white shadow-sm" style="width:38px;height:38px; cursor: pointer;" data-bs-toggle="dropdown">
                @if(App::getLocale() == 'en')
                    <img src="https://flagcdn.com/w20/gb.png" alt="EN" style="border-radius: 2px;">
                @else
                    <img src="https://flagcdn.com/w20/vn.png" alt="VI" style="border-radius: 2px;">
                @endif
            </div>
            <ul class="dropdown-menu dropdown-menu-end shadow border-0 mt-2" style="min-width: 120px; border-radius: 12px; z-index: 1050;">
                <li><a class="dropdown-item py-2 {{ App::getLocale() == 'vi' ? 'fw-bold bg-light' : '' }}" href="{{ route('lang.switch', 'vi') }}"><img src="https://flagcdn.com/w20/vn.png" class="me-2"> Tiếng Việt</a></li>
                <li><a class="dropdown-item py-2 {{ App::getLocale() == 'en' ? 'fw-bold bg-light' : '' }}" href="{{ route('lang.switch', 'en') }}"><img src="https://flagcdn.com/w20/gb.png" class="me-2"> English</a></li>
            </ul>
        </div>
    </div>
</div>
...
<div class="category-scroller">
    <div class="cat-item active" onclick="filterCategory('all', this)">{{ __('messages.all_categories') }}</div>
...
<div id="menuList">
    ...
    @empty
        <div class="text-center py-5 opacity-50"><p>{{ __('messages.loading_dishes') }}</p></div>
    @endforelse
</div>
...
<div class="floating-cart animate__animated" id="floatingCart" onclick="showCartModal()">
    <div class="d-flex align-items-center">
        ...
        <div>
            <div class="fw-bold" style="font-size: 0.85rem;">{{ __('messages.view_order') }}</div>
            <small class="text-white-50" id="totalCountText" style="font-size: 0.7rem;">0 {{ __('messages.items_selected') }}</small>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-dialog-centered mx-auto" style="max-width: 92%;">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 30px;">
            <div class="modal-header border-0 pb-0 pt-4 px-4">
                <h5 class="modal-title fw-800">{{ __('messages.cart') }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-4">
                <div id="cartDetails" style="max-height: 30vh; overflow-y: auto;" class="mb-4"></div>
                <div class="mb-3">
                    <label class="form-label fw-bold small text-muted">{{ __('messages.note_for_shop') }}</label>
                    <textarea id="customer_note" class="form-control border-0 bg-light" rows="2" placeholder="{{ __('messages.note_placeholder') }}" style="border-radius: 15px; font-size: 0.85rem;"></textarea>
                </div>
                <div class="d-flex justify-content-between align-items-center pt-3 border-top">
                    <span class="fw-bold text-secondary">{{ __('messages.grand_total_cart') }}</span>
                    <span class="fw-800 fs-2 text-danger" id="modalTotalPrice">0đ</span>
                </div>
            </div>
            <div class="p-4 pt-0">
                <button class="btn btn-warning w-100 py-3 rounded-pill fw-800" id="btnConfirm" onclick="sendOrder()">{{ __('messages.confirm_order_btn') }}</button>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-dialog-centered mx-auto" style="max-width: 85%;">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 30px;">
            <div class="modal-body text-center py-5">
                ...
                <h4 class="fw-800 mb-2">{{ __('messages.order_success_title') }}</h4>
                <p class="text-muted mb-4 small">{{ __('messages.order_success_desc') }}</p>
                <button class="btn-back-menu w-100" onclick="location.reload()">{{ __('messages.continue') }}</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function refreshUI() {
        ...
        if (totalQty > 0) {
            ...
            document.getElementById('totalCountText').textContent = `${totalQty} {{ __('messages.items_selected') }}`;
    ...
    async function sendOrder() {
        ...
        } catch (e) { alert("{{ __('messages.server_error') }}"); btn.disabled = false; }
    }
