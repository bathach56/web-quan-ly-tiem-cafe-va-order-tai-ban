...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        /* Responsive Fix for Zooming / Small Screens */
        @media (max-height: 900px), (max-width: 1366px) {
            ...
            .promo-strip { padding: 8px 15px; }
            .promo-strip .key { height: 32px !important; font-size: 0.7rem !important; }
            .totals-area { padding: 10px 15px; }
            .totals-area .mb-3 { margin-bottom: 8px !important; }
            .totals-area .mb-2 { margin-bottom: 4px !important; }
            #order_note { height: 34px !important; min-height: 34px !important; padding: 4px 10px; font-size: 0.8rem; }
            .total-main .val { font-size: 1.4rem; }
            .pay-method-btn { height: 36px; font-size: 0.7rem; }
            .cash-display { height: 40px; font-size: 1.2rem; margin-bottom: 8px; }
            .keypad { padding-bottom: 10px; gap: 4px; }
            .key { height: 38px; font-size: 0.9rem; }
            .btn-checkout { height: 48px; font-size: 0.9rem; border-radius: 12px; }
            .product-grid { gap: 10px; grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)); }
            .product-card .p-3 { padding: 8px !important; }
            .product-img-wrap { aspect-ratio: 16/9; }
        }

        @media (max-width: 1200px) {
            .pos-wrapper { grid-template-columns: 1fr 360px; }
            .btn-table span { display: none; } /* Hide "BÀN" text, keep icon */
            .btn-takeaway { padding: 0 10px; font-size: 0.85rem; }
        }
        @media (max-width: 991px) {
            .pos-wrapper { display: flex; flex-direction: column; overflow-y: auto; height: 100vh; }
            body { overflow: hidden; }
            .pos-left { flex: 1; min-height: 50vh; overflow: hidden; }
            .pos-right { flex: 1; height: auto; min-height: 50vh; border-top: 2px solid var(--amber); }
            .pos-topbar { flex-wrap: wrap; }
            .search-wrap { order: 3; width: 100%; margin-top: 10px; margin-left: 0 !important; }
            .d-flex.gap-2.ms-3 { margin-left: auto !important; }
        }
    </style>
</head>
<body>
...
<div class="pos-wrapper">
    ...
    <div class="pos-left">
        <div class="pos-topbar">
            ...
            <div class="search-wrap ms-2">
                ...
                <input type="text" id="productSearch" placeholder="{{ __('messages.search_product_pos') }}" onkeyup="filterProducts()">
            </div>
            <div class="d-flex gap-2 ms-3 align-items-center">
                <div class="dropdown me-1">
                    <div class="d-flex align-items-center gap-2" data-bs-toggle="dropdown" style="cursor: pointer; padding: 5px 10px; border-radius: 8px; border: 1.5px solid var(--border-light); height: 44px;">
                        @if(App::getLocale() == 'en')
                            <img src="https://flagcdn.com/w20/gb.png" alt="English" style="border-radius: 2px;"> <span class="small fw-bold text-dark">EN</span>
                        @else
                            <img src="https://flagcdn.com/w20/vn.png" alt="Tiếng Việt" style="border-radius: 2px;"> <span class="small fw-bold text-dark">VI</span>
                        @endif
                    </div>
                    <ul class="dropdown-menu dropdown-menu-end shadow border-0 mt-2 animate__animated animate__fadeIn" style="min-width: 120px; z-index: 1050;">
                        <li><a class="dropdown-item py-2 {{ App::getLocale() == 'vi' ? 'active bg-warning text-dark fw-bold' : '' }}" href="{{ route('lang.switch', 'vi') }}"><img src="https://flagcdn.com/w20/vn.png" class="me-2"> Tiếng Việt</a></li>
                        <li><a class="dropdown-item py-2 {{ App::getLocale() == 'en' ? 'active bg-warning text-dark fw-bold' : '' }}" href="{{ route('lang.switch', 'en') }}"><img src="https://flagcdn.com/w20/gb.png" class="me-2"> English</a></li>
                    </ul>
                </div>

                <button class="btn-takeaway" onclick="setTakeaway()">
                    <i class="fa-solid fa-bag-shopping"></i> {{ __('messages.takeaway') }}
                </button>
                <button class="btn-table" data-bs-toggle="modal" data-bs-target="#tableModal" id="btnMainTable">
                    <i class="fa-solid fa-couch"></i> <span>{{ __('messages.table') }}</span>
        ...
        <div class="cat-row">
            <button class="cat-btn active" onclick="filterCategory('all', this)">{{ __('messages.all_categories') }}</button>
    ...
    <div class="pos-right">
        <div class="cart-header d-flex justify-content-between align-items-center">
            <div><div class="text-warning fw-900" id="selectedTableName">{{ __('messages.select_table_or_takeaway') }}</div><small class="opacity-50">{{ __('messages.staff_name') }}: {{ Auth::user()->name }}</small></div>
            <button class="qty-btn" style="color:var(--danger)" onclick="confirmReset()"><i class="fa-solid fa-trash-can"></i></button>
        </div>

        <div class="cart-list" id="cartContent"><div class="text-center mt-5 opacity-25"><p>{{ __('messages.empty_cart') }}</p></div></div>

        <div class="promo-strip">
            <div style="flex:1"><small class="text-white-50 d-block mb-1" style="font-size:0.6rem">{{ __('messages.discount_percent') }}</small><input type="number" id="manual_discount" class="key w-100" style="background:var(--panel-light); border:1px solid var(--panel-border); height:38px; color:white; text-align:center;" placeholder="0" onchange="updateTotal()" readonly></div>
            <div style="flex:1"><small class="text-white-50 d-block mb-1" style="font-size:0.6rem">{{ __('messages.voucher_pos') }}</small><button class="key w-100" style="background:var(--panel-light); border:1px solid var(--panel-border); height:38px; color:var(--amber); font-size:0.7rem;" data-bs-toggle="modal" data-bs-target="#voucherModal"><span id="selectedVoucherLabel" class="text-truncate px-2">{{ __('messages.select_voucher') }}</span></button><input type="hidden" id="selected_voucher_code"></div>
        </div>

        <div class="totals-area">
            <div class="d-flex justify-content-between small opacity-75 mb-1"><span>{{ __('messages.subtotal') }}</span> <span id="subtotalVal">0đ</span></div>
            <div class="d-flex justify-content-between small text-danger mb-2"><span>{{ __('messages.discount') }}</span> <span id="discountVal">-0đ</span></div>
            
            <!-- Ghi chú đơn hàng -->
            <div class="mb-3">
                <small class="text-white-50 d-block mb-1" style="font-size: 0.7rem;">{{ __('messages.order_note') }}</small>
                <textarea id="order_note" class="form-control bg-dark text-white border-secondary" rows="2" style="font-size: 0.8rem; border-radius: 10px; border-color: var(--panel-border)" readonly></textarea>
            </div>

            <div class="total-main d-flex justify-content-between align-items-center mb-3"><span class="fw-bold">{{ __('messages.total_amount') }}</span><span class="val" id="totalAmount">0đ</span></div>

            <div class="pay-methods">
                <button class="pay-method-btn active" onclick="setPayment('cash', this)">{{ __('messages.cash_method') }}</button>
                <button class="pay-method-btn" onclick="setPayment('card', this)">{{ __('messages.card_pos_method') }}</button>
                <button class="pay-method-btn" onclick="setPayment('banking', this)">{{ __('messages.banking_method') }}</button>
            </div>
            ...
            <div class="keypad">
                ...
                <button class="key" style="grid-column: span 2;" onclick="pressKey('000')">.000</button><button class="key k-exact" style="grid-column: span 2;" onclick="quickCash()">{{ __('messages.exact_amount') }}</button>
            </div>

            <button class="btn-checkout" id="btnCheckout" onclick="checkout()">{{ __('messages.confirm_payment') }}</button>
        </div>
    </div>
</div>
...
<div class="modal fade" id="tableModal" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content" style="border-radius: 25px; overflow: hidden;">
            <div class="modal-header bg-dark text-white p-4 border-0"><h5 class="fw-800 m-0 text-uppercase">{{ __('messages.serving_table_map') }}</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-6 col-md-4 col-lg-3">
                        <div class="table-box p-4 text-center border-2 border rounded-4 bg-white position-relative {{ $table->status === 'occupied' ? 'border-danger text-danger' : 'border-success text-success' }}" style="cursor:pointer" id="table-{{ $table->id }}" onclick="selectOrderTable('{{ $table->id }}', '{{ $table->name }}')">
                            ...
                            <i class="fa-solid fa-mug-hot fa-2x mb-2"></i><div class="fw-bold">{{ $table->name }}</div><small class="text-uppercase status-label" style="font-size:0.6rem">{{ $table->status === 'occupied' ? __('messages.has_guests') : __('messages.empty') }}</small>
                        </div>
                    </div>
                    @endforeach
                </div>
...
<div class="modal fade" id="voucherModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content bg-dark text-white" style="border-radius: 20px;">
            <div class="modal-header border-secondary p-4"><h5 class="fw-800 m-0 text-warning">{{ __('messages.available_vouchers') }}</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
            <div class="modal-body p-0" style="max-height: 400px; overflow-y: auto;">
                ...
                @empty <div class="p-5 text-center opacity-25"><p>{{ __('messages.empty_list') }}</p></div> @endforelse
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function setTakeaway() {
        ...
        $('#order_note').val('');
        renderCart();
        $('#selectedTableName').text('{{ __('messages.takeaway_customer') }}').addClass('text-info text-uppercase').removeClass('text-warning');
        $('#topbarTableLabel').text('{{ __('messages.takeaway') }}');
        showToast("{{ __('messages.takeaway_mode') }}");
    }
    ...
    function selectOrderTable(id, name) {
        ...
        $.get(`{{ url('/pos/table-order') }}/${id}`, function(res) {
            if (res.success && res.cart) { 
                ...
                renderCart(); 
                showToast("{{ __('messages.order_loaded') }}"); 
            }
    ...
    function renderCart() {
        ...
        $('#cartContent').html(cart.length ? html : '<div class="text-center mt-5 opacity-25"><p>{{ __('messages.empty_cart') }}</p></div>');
    ...
    function applyVoucher(code, val, type, min) {
        ...
        if (subtotal < minOrder) { alert(`{{ __('messages.not_enough_min_order') }} ${minOrder.toLocaleString()}đ!`); return; }
        ...
        updateTotal();
        showToast("{{ __('messages.apply_voucher_success') }}");
    }
    ...
    function updateTotal() {
        ...
        if (voucherData.code) {
            ...
            if (voucherData.type === 'percentage') {
                $('#manual_discount').val(voucherData.value);
            } else {
                $('#manual_discount').val(0);
            }
        } else {
            $('#manual_discount').val(0);
        }
        totalValue = Math.max(0, subtotal - discountVal);
        $('#discountVal').text('-' + Math.round(discountVal).toLocaleString() + 'đ');
    ...
    function checkout() {
        if (!selectedTableId || cart.length === 0) { alert("{{ __('messages.missing_info') }}"); return; }
        const btn = $('#btnCheckout'); btn.prop('disabled', true).text('{{ __('messages.processing') }}');
        $.ajax({
            ...
            success: function(res) {
                if (res.success) {
                    showToast('{{ __('messages.payment_success') }}');
                    window.open(`{{ url('/pos/print-receipt') }}/${res.order_id}`, '_blank');
                    setTimeout(() => location.reload(), 1000);
                } else { alert("{{ __('messages.error_prefix') }}" + res.message); btn.prop('disabled', false).text('{{ __('messages.confirm_payment') }}'); }
            },
            error: function(xhr) { alert("{{ __('messages.server_error') }}"); btn.prop('disabled', false).text('{{ __('messages.confirm_payment') }}'); }
        });
    }

    function confirmReset() { if(confirm('{{ __('messages.cancel_order_confirm') }}')) { cart = []; renderCart(); } }
        ...
        $.get("{{ route('tables.fetch_status') }}", function(tables) {
            ...
            tables.forEach(table => {
                ...
                if (table.status === 'occupied') {
                    ...
                    el.removeClass('border-success text-success').addClass('border-danger text-danger'); el.find('.status-label').text('{{ __('messages.has_guests') }}'); b.removeClass('d-none');
