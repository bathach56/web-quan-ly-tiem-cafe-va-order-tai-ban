...
<div class="container-fluid pb-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        <div>
            <h3 class="fw-bold m-0 dynamic-text"><i class="fa-solid fa-chart-line text-primary me-2"></i> {{ __('messages.revenue_report') }}</h3>
            <p class="text-secondary small m-0 mt-1">{{ __('messages.management_system') }} {{ $shop_setting->shop_name ?? 'HUTECH Coffee' }}</p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('reports.pdf', ['start_date' => $startDate, 'end_date' => $endDate]) }}" 
               class="btn btn-danger btn-sm px-3 fw-bold shadow-sm" style="background: #ef4444; border: none;">
                <i class="fa-solid fa-file-pdf me-1"></i> {{ __('messages.export_pdf') }}
            </a>
            ...
            <a href="{{ route('reports.excel', ['start_date' => $startDate, 'end_date' => $endDate]) }}" 
               class="btn btn-success btn-sm px-3 fw-bold shadow-sm" style="background: #10b981; border: none;">
                <i class="fa-solid fa-file-excel me-1"></i> {{ __('messages.export_excel') }}
            </a>
        </div>
    </div>
    ...
    <div class="report-card p-4 mb-4">
        <form action="{{ route('reports.index') }}" method="GET">
            <div class="row g-3 align-items-end">
                <div class="col-md-4">
                    <label class="small fw-bold mb-2 dynamic-text text-uppercase">{{ __('messages.from_date') }}</label>
                    <input type="date" name="start_date" class="form-control date-input" value="{{ $startDate }}">
                </div>
                <div class="col-md-4">
                    <label class="small fw-bold mb-2 dynamic-text text-uppercase">{{ __('messages.to_date') }}</label>
                ...
                <div class="col-md-4">
                    <button type="submit" class="btn btn-primary w-100 fw-bold py-2 shadow-sm text-uppercase">
                        <i class="fa-solid fa-filter me-2"></i> {{ __('messages.update_report') }}
                    </button>
                </div>
            </div>
        </form>
    ...
    <div class="row g-4 mb-4">
        <div class="col-md-6">
            <div class="summary-card-revenue animate__animated animate__fadeInLeft">
                <h6 class="fw-bold mb-2" style="letter-spacing: 1px; opacity: 0.9;">{{ __('messages.total_revenue_period') }}</h6>
                <h1 class="fw-bold m-0" style="font-size: 2.5rem;">{{ number_format($totalRevenue) }} <span style="font-size: 1.5rem;">{{ __('messages.vnd') }}</span></h1>
            </div>
        </div>
        <div class="col-md-6">
            <div class="summary-card-orders animate__animated animate__fadeInRight">
                <h6 class="fw-bold mb-2" style="letter-spacing: 1px; opacity: 0.9;">{{ __('messages.total_orders') }}</h6>
                <h1 class="fw-bold m-0" style="font-size: 2.5rem;">{{ number_format($totalOrders) }} <span style="font-size: 1.5rem;">{{ __('messages.orders') }}</span></h1>
            </div>
        </div>
    </div>
        ...
        <div class="col-lg-8">
            <div class="report-card overflow-hidden p-0 animate__animated animate__fadeInUp">
                <div class="p-3 border-bottom border-color fw-bold dynamic-text bg-light bg-opacity-10">
                    <i class="fa-solid fa-list me-2"></i> {{ __('messages.revenue_details_by_date') }}
                </div>
                <div class="table-responsive">
                    <table class="table report-table mb-0 text-center">
                        <thead>
                            <tr>
                                <th class="ps-4 text-start fw-bold">{{ __('messages.transaction_date') }}</th>
                                <th class="fw-bold">{{ __('messages.order_count') }}</th>
                                <th class="pe-4 text-end fw-bold">{{ __('messages.daily_revenue') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="pe-4 text-end fw-bold text-success">{{ number_format($report->daily_revenue) }} {{ __('messages.currency') }}</td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="3" class="text-center py-5 text-secondary">
                                    <i class="fa-solid fa-box-open fa-3x mb-3 opacity-50"></i>
                                    <p class="mb-0">{{ __('messages.no_data_in_period') }}</p>
                                </td>
                            </tr>
                            @endforelse
                            ...
                            <tr style="background: rgba(0,0,0,0.05);">
                                <td class="ps-4 text-start fw-bold dynamic-text fs-5">{{ __('messages.grand_total') }}</td>
                                <td class="fw-bold dynamic-text fs-4 text-info">{{ number_format($totalOrders) }}</td>
                                <td class="pe-4 text-end fw-bold fs-4" style="color: #10b981;">{{ number_format($totalRevenue) }} {{ __('messages.currency') }}</td>
                            </tr>
                            @endif
                        </tbody>
                    </table>
        ...
        <div class="col-lg-4">
            <div class="report-card animate__animated animate__fadeInUp" style="animation-delay: 0.2s;">
                <div class="p-3 border-bottom border-color fw-bold text-warning bg-light bg-opacity-10">
                    <i class="fa-solid fa-crown me-2"></i> {{ __('messages.top_selling_items') }}
                </div>
                <div class="card-body">
                    ...
                    <div class="product-rank-item d-flex align-items-center justify-content-between">
                        ...
                        <span class="text-info fw-bold">{{ $product->total_qty }} <small>{{ __('messages.items') }}</small></span>
                    </div>
                    @empty
                    <p class="text-center text-muted py-4 small">{{ __('messages.no_data') }}</p>
                    @endforelse
                </div>
            </div>
        </div>
