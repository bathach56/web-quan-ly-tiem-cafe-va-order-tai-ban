...
<body data-theme="dark">
    ...
    <nav class="sidebar" id="sidebar">
        ...
        <div class="nav-label" style="padding: 20px 25px 10px; font-size: 0.65rem; color: var(--primary); text-transform: uppercase; font-weight: 700; opacity: 0.8;">{{ __('messages.report_sales') }}</div>
        ...
        <a href="{{ route('reports.index') }}" class="{{ request()->is('reports*') ? 'active' : '' }}">
            <i class="fa-solid fa-file-invoice-dollar"></i> {{ __('messages.order_history') }}
        </a>

        <div class="nav-label" style="padding: 20px 25px 10px; font-size: 0.65rem; color: var(--primary); text-transform: uppercase; font-weight: 700; opacity: 0.8;">{{ __('messages.menu_management') }}</div>
        <a href="{{ route('products.index') }}" class="{{ request()->is('products*') ? 'active' : '' }}">
            <i class="fa-solid fa-mug-saucer"></i> {{ __('messages.products') }}
        </a>
        <a href="{{ route('categories.index') }}" class="{{ request()->is('categories*') ? 'active' : '' }}">
            <i class="fa-solid fa-tags"></i> {{ __('messages.categories') }}
        </a>
        <a href="{{ route('inventory.index') }}" class="{{ request()->is('inventory*') ? 'active' : '' }}">
            <i class="fa-solid fa-warehouse"></i> {{ __('messages.inventory') }}
        </a>
        <a href="{{ route('vouchers.index') }}" class="{{ request()->is('vouchers*') ? 'active' : '' }}">
            <i class="fa-solid fa-ticket-simple"></i> {{ __('messages.vouchers') }}
        </a>
        <a href="{{ route('tables.index') }}" class="{{ request()->is('tables*') ? 'active' : '' }}">
            <i class="fa-solid fa-couch"></i> {{ __('messages.floor_plan') }}
        </a>

        <div class="nav-label" style="padding: 20px 25px 10px; font-size: 0.65rem; color: var(--primary); text-transform: uppercase; font-weight: 700; opacity: 0.8;">{{ __('messages.operations_hr') }}</div>
        <a href="{{ route('employees.index') }}" class="{{ request()->is('employees*') ? 'active' : '' }}">
            <i class="fa-solid fa-users-gear"></i> {{ __('messages.employees') }}
        </a>
        <a href="{{ route('settings.index') }}" class="{{ request()->is('settings*') ? 'active' : '' }}">
            <i class="fa-solid fa-gears"></i> {{ __('messages.shop_settings') }}
        </a>
    </nav>
    ...
    <div class="main-content">
        <header class="topbar">
            ...
            <div class="d-flex align-items-center gap-3">
                <!-- LANGUAGE SWITCHER -->
                <div class="dropdown">
                    <div class="d-flex align-items-center gap-2" data-bs-toggle="dropdown" style="cursor: pointer; color: var(--primary);">
                        @if(App::getLocale() == 'en')
                            <img src="https://flagcdn.com/w20/gb.png" alt="English" style="border-radius: 3px;"> <span class="d-none d-sm-block small fw-bold">EN</span>
                        @else
                            <img src="https://flagcdn.com/w20/vn.png" alt="Tiếng Việt" style="border-radius: 3px;"> <span class="d-none d-sm-block small fw-bold">VI</span>
                        @endif
                    </div>
                    <ul class="dropdown-menu dropdown-menu-end shadow border-0 mt-3 animate__animated animate__fadeIn" style="min-width: 120px;">
                ...
                <i class="fa-solid fa-moon fs-5 border-start ps-3" id="theme-toggle" style="cursor:pointer; color: var(--primary);" title="Chuyển chế độ Sáng/Tối"></i>
                ...
                <div class="dropdown border-start ps-3">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end shadow border-0 mt-3 animate__animated animate__fadeIn">
                        <li><a class="dropdown-item py-2" href="{{ route('profile.index') }}"><i class="fa-solid fa-user me-2"></i> {{ __('messages.profile') }}</a></li>
                        <li><hr class="dropdown-divider"></li>
                        <li>
                            <form action="{{ route('logout') }}" method="POST">
                                @csrf
                                <button type="submit" class="dropdown-item py-2 text-danger fw-bold border-0 bg-transparent w-100 text-start">
                                    <i class="fa-solid fa-power-off me-2"></i> {{ __('messages.logout') }}
                                </button>
                            </form>
                        </li>
                    </ul>
