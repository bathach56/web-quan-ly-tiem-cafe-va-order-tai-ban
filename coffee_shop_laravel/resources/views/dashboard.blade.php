...
<div class="container-fluid pb-5">
    ...
    <div class="welcome-banner mb-4 d-flex justify-content-between align-items-center flex-wrap gap-4">
        <div>
            <h2 class="fw-800 mb-2 animate__animated animate__fadeInLeft" id="greeting">{{ __('messages.loading') }}</h2>
        ...
        <div class="clock-box animate__animated animate__fadeInRight">
            ...
            <div class="clock-date mt-2 opacity-75 small" id="clockDate">{{ __('messages.date_placeholder') }}</div>
        </div>
    </div>
    ...
    <div class="row g-4 mb-4">
        ...
        <div class="col-12 col-sm-6 col-xl-3">
            <div class="stat-card sky">
                ...
                <div class="stat-label">{{ __('messages.occupied_tables') }}</div>
            </div>
        </div>
        <div class="col-12 col-sm-6 col-xl-3">
            <div class="stat-card rose">
                ...
                <div class="stat-label">{{ __('messages.menu_categories') }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="row g-4">
        <div class="col-12 col-lg-8">
            <div class="chart-card">
                <div class="chart-header">
                    <div class="chart-title"><i class="fa-solid fa-chart-line text-warning me-2"></i>{{ __('messages.revenue_chart_7_days') }}</div>
                    <span class="badge bg-warning bg-opacity-10 text-warning px-3 py-2 rounded-pill">{{ __('messages.unit_vnd') }}</span>
                </div>
        ...
        <div class="col-12 col-lg-4">
            <div class="chart-card">
                <div class="chart-header">
                    <div class="chart-title"><i class="fa-solid fa-chart-pie text-success me-2"></i>{{ __('messages.sales_by_category') }}</div>
                </div>
...
<script>
    ...
    function updateHeader() {
        ...
        const name = "{{ auth()->user()->name ?? __('messages.group_3_member') }}";
        let greet = "{{ __('messages.good_evening') }}";
        
        if (hour < 12) greet = "{{ __('messages.good_morning') }}";
        else if (hour < 18) greet = "{{ __('messages.good_afternoon') }}";
        
        const localeStr = "{{ App::getLocale() == 'en' ? 'en-US' : 'vi-VN' }}";
        document.getElementById('greeting').textContent = `${greet}, ${name}! 👋`;
        document.getElementById('clock').textContent = now.toLocaleTimeString(localeStr, { hour12: false });
        document.getElementById('clockDate').textContent = now.toLocaleDateString(localeStr, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    }
